...
public class GameOfLife implements ActionListener, Runnable {
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == randomCaseButton) {
            deja_todo_lindo();
            cellArray = caso_random();
            cellArrayCopy = cellArray;
            mainThread.start();
        } else if (ae.getSource() == fileCaseButton) {
            deja_todo_lindo();
            filePathString = filePathTextField.getText();
            cellArray = caso_de_archivo(filePathString);
            cellArrayCopy = cellArray;
            mainThread.start();
    ...
    public void run() {
        while (!(stopFlag)) {
            ...
            cellNumberLabel.setText("\t" + "Número de celulas: " + countCellsAliveRecursiveMethod(countCellsAliveIterativeMethod(cellArray)) + "\t");
        }
